...
the module's behavioral signals. The matcher is advisory, not blocking --
a low score triggers investigation, not automatic rollback.

**`refactorType: 'module'`**

1. Collect the file lists:
   - **beforeFiles**: the original module file
   - **afterFiles**: the modified module + any new split files (e.g.,
     `.logic.ts` for pure core extraction)
