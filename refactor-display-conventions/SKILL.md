...
| BFF raw coercion (gmork bypass) | Replace `?? 0`, `\|\| 0`, `!= null ? Number(x) : 0`, `parseInt(x) \|\| 0` with `gmork(key, rawValue)`. Add `import { gmork } from '@/server/lib/gmork'`. Add a theNothing entry in `src/shared/constants/theNothing.ts` if missing. Key format: `'domain.fieldName'`. Rule: `notApplicable` if null should display as "-", `measuredZero` if 0 is valid. |
