...
### Step 6b: Intention matcher (post-verification)

After tsc passes, run the intention matcher to verify the refactor preserved
the hook's behavioral signals. The matcher is advisory, not blocking -- a
low score triggers investigation, not automatic rollback.
