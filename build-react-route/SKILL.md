...
2. Run `npx tsx scripts/AST/ast-query.ts authz <generated-files> --pretty`.
   Flag any `RAW_ROLE_CHECK` observations -- new routes must use the canonical
...
6. Verify the container handles all behavioral concerns:
   ...
   ```bash
   npx tsx scripts/AST/ast-query.ts errors <new-container-file> --pretty
   # Expect: 0 QUERY_ERROR_UNHANDLED, 0 MUTATION_ERROR_UNHANDLED

   npx tsx scripts/AST/ast-query.ts concerns <new-container-file> --pretty
