...
- Identify the API endpoint path for this domain. Service hooks hardcode their
  API path as a string literal (e.g., `'/users/teams/getByOrgId'`). Do NOT
  import path helpers from `urlsRegistry.ts` -- that file contains page route
  helpers only.
...
- For mutations: accept an optional `options` parameter for caller callback
  forwarding. The hook's own `onSuccess` (cache invalidation) runs first,
  then forwards to the caller's `onSuccess`. Pattern:
  ```ts
  export function useDoThingMutation(options?: MutationOpts<ResponseType, Variables>) {
    // ...
    return useMutation({
      mutationFn: ...,
