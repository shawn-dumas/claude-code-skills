...
export function analyzeHandlerStructure(filePath: string): HandlerStructureAnalysis {
  ...
  if (handlerNode) {
    ...
    if (Node.isFunctionDeclaration(handlerNode)) {
      ...
      bodyText = handlerNode.getBody().getText();
      /* v8 ignore start -- defensive: findHandlerFunction only returns FunctionDeclaration/ArrowFunction/FunctionExpression */
    } else {
      bodyText = '';
    }
    /* v8 ignore stop */
...
export const cliConfig: ObservationToolConfig<HandlerStructureAnalysis> = {
  cacheNamespace: 'ast-handler-structure',
  helpText: HELP_TEXT,
  analyzeFile: analyzeHandlerStructure,
  analyzeDirectory: analyzeHandlerStructureDirectory,
};
