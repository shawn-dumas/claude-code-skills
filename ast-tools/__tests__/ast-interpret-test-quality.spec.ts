...
describe('ast-interpret-test-quality', () => {
  ...
  describe('Helper resolution', () => {
    ...
    const helperFixture = path.join(fixtureDir, 'test-quality-helper.ts');

    describe('analyzeTestHelper', () => {
      it('extracts assertion counts and classifications from helper functions', () => {
        const entries = analyzeTestHelper(helperFixture);

        expect(entries.length).toBe(3);

        const userCard = entries.find(e => e.functionName === 'expectUserCard');
        expect(userCard).toBeDefined();
        expect(userCard!.assertionCount).toBe(2);
        expect(userCard!.userVisibleCount).toBe(2);
        expect(userCard!.implementationCount).toBe(0);
      });

      it('classifies toHaveBeenCalled as implementation-detail', () => {
        const entries = analyzeTestHelper(helperFixture);

        const callback = entries.find(e => e.functionName === 'expectCallbackFired');
        expect(callback).toBeDefined();
        expect(callback!.assertionCount).toBe(2);
        expect(callback!.userVisibleCount).toBe(0);
        expect(callback!.implementationCount).toBe(2);
      });

      it('classifies toBe as implementation-detail', () => {
        const entries = analyzeTestHelper(helperFixture);

        const dataShape = entries.find(e => e.functionName === 'expectDataShape');
        expect(dataShape).toBeDefined();
        expect(dataShape!.assertionCount).toBe(2);
        expect(dataShape!.userVisibleCount).toBe(0);
        expect(dataShape!.implementationCount).toBe(2);
      });

      it('returns empty array for non-existent file', () => {
        const entries = analyzeTestHelper('/nonexistent/path.ts');
        expect(entries).toHaveLength(0);
      });
    });

    describe('buildTestHelperIndex', () => {
      it('builds index from TEST_HELPER_DELEGATION observations', () => {
        const delegations: TestObservation[] = [
          makeObservation('TEST_HELPER_DELEGATION', 'test.spec.ts', 10, {
            delegationType: 'helper',
            functionName: 'expectUserCard',
            argCount: 1,
            isImported: true,
            sourceFile: './test-quality-helper',
          }),
        ];

        const index = buildTestHelperIndex(delegations, fixtureDir);

        expect(index.entries.size).toBeGreaterThan(0);
        const entry = index.entries.get('expectUserCard');
        expect(entry).toBeDefined();
        expect(entry!.userVisibleCount).toBe(2);
      });

      it('returns empty index when no imported delegations', () => {
        const delegations: TestObservation[] = [
          makeObservation('TEST_HELPER_DELEGATION', 'test.spec.ts', 10, {
            delegationType: 'helper',
            functionName: 'localHelper',
            argCount: 0,
            isImported: false,
          }),
        ];
    ...
    describe('interpreter integration with helper index', () => {
      it('resolves helper delegation and attributes user-visible assertions', () => {
        const helperIndex: TestHelperIndex = {
          entries: new Map([
            [
              'expectUserCard',
              {
                functionName: 'expectUserCard',
                file: 'test-quality-helper.ts',
                line: 12,
                assertionCount: 2,
                userVisibleCount: 2,
        ...
        const observations: TestObservation[] = [
          makeObservation('TEST_BLOCK', 'test.spec.ts', 5, { testName: 'renders user card' }),
          makeObservation('TEST_HELPER_DELEGATION', 'test.spec.ts', 8, {
            delegationType: 'helper',
            functionName: 'expectUserCard',
            argCount: 1,
            isImported: true,
        ...
        const result = interpretTestQuality(observations, astConfig, '', true, helperIndex);

        const userVisible = result.assessments.filter(a => a.kind === 'ASSERTION_USER_VISIBLE');
        expect(userVisible).toHaveLength(1);
        expect(userVisible[0].rationale[0]).toContain('expectUserCard');
        expect(userVisible[0].rationale[0]).toContain('2 user-visible');
        expect(userVisible[0].confidence).toBe('medium');
      });

      it('resolves helper delegation and attributes implementation assertions', () => {
        const helperIndex: TestHelperIndex = {
          entries: new Map([
            [
              'expectCallbackFired',
              {
                functionName: 'expectCallbackFired',
                file: 'test-quality-helper.ts',
                line: 18,
                assertionCount: 2,
                userVisibleCount: 0,
        ...
        const observations: TestObservation[] = [
          makeObservation('TEST_BLOCK', 'test.spec.ts', 5, { testName: 'fires callback' }),
          makeObservation('TEST_HELPER_DELEGATION', 'test.spec.ts', 8, {
            delegationType: 'helper',
            functionName: 'expectCallbackFired',
            argCount: 1,
        ...
        const implementation = result.assessments.filter(a => a.kind === 'ASSERTION_IMPLEMENTATION');
        expect(implementation).toHaveLength(1);
        expect(implementation[0].rationale[0]).toContain('expectCallbackFired');
        expect(implementation[0].rationale[0]).toContain('2 implementation');
      });

      it('emits requiresManualReview for unresolved imported helper', () => {
        // No helper index provided, but delegation is imported
        const observations: TestObservation[] = [
          makeObservation('TEST_BLOCK', 'test.spec.ts', 5, { testName: 'test' }),
          makeObservation('TEST_HELPER_DELEGATION', 'test.spec.ts', 8, {
            delegationType: 'helper',
            functionName: 'unknownHelper',
            argCount: 1,
            isImported: true,
            sourceFile: './nonexistent-helpers',
          }),
        ];

        const result = interpretTestQuality(observations, astConfig, '', true);

        const unresolved = result.assessments.filter(
          a => a.kind === 'ASSERTION_IMPLEMENTATION' && a.requiresManualReview,
        );
        expect(unresolved).toHaveLength(1);
        expect(unresolved[0].confidence).toBe('low');
        expect(unresolved[0].rationale[0]).toContain('unresolved helper delegation');
        expect(unresolved[0].rationale[0]).toContain('unknownHelper');
      });

      it('skips local (non-imported) helper delegations', () => {
        const observations: TestObservation[] = [
          makeObservation('TEST_BLOCK', 'test.spec.ts', 5, { testName: 'test' }),
          makeObservation('TEST_HELPER_DELEGATION', 'test.spec.ts', 8, {
            delegationType: 'helper',
        ...
        // Should NOT produce assertion assessments for local helpers
        // (their assertions are already counted inline)
        const helperAssertions = result.assessments.filter(
          a =>
            (a.kind === 'ASSERTION_USER_VISIBLE' || a.kind === 'ASSERTION_IMPLEMENTATION') &&
            a.subject.symbol === 'localHelper',
        );
        expect(helperAssertions).toHaveLength(0);
      });

      it('attributes both user-visible and implementation from same helper', () => {
        const helperIndex: TestHelperIndex = {
          entries: new Map([
            [
              'verifyForm',
              {
                functionName: 'verifyForm',
                file: 'helpers.ts',
                line: 5,
                assertionCount: 4,
                userVisibleCount: 2,
        ...
        const userVisible = result.assessments.filter(
          a => a.kind === 'ASSERTION_USER_VISIBLE' && a.subject.symbol === 'verifyForm',
        );
        const implementation = result.assessments.filter(
          a => a.kind === 'ASSERTION_IMPLEMENTATION' && a.subject.symbol === 'verifyForm',
        );
        expect(userVisible).toHaveLength(1);
        expect(implementation).toHaveLength(1);
      });
    ...
    describe('end-to-end with fixture files', () => {
      it('resolves helpers from real fixture test file', () => {
        const testFilePath = path.join(fixtureDir, 'test-quality-with-helper.spec.ts');

        const analysis = analyzeTestFile(testFilePath);

        // Build helper index from the observations
        ...
        const helperIndex = buildTestHelperIndex(delegations, fixtureDir);

        const result = interpretTestQuality(analysis.observations, astConfig, '', analysis.subjectExists, helperIndex);

        // Should have user-visible assertions from expectUserCard helper
        const userVisible = result.assessments.filter(
          a => a.kind === 'ASSERTION_USER_VISIBLE' && a.subject.symbol === 'expectUserCard',
        );
        expect(userVisible).toHaveLength(1);

        // Should have implementation assertions from expectCallbackFired helper
        const implementation = result.assessments.filter(
          a => a.kind === 'ASSERTION_IMPLEMENTATION' && a.subject.symbol === 'expectCallbackFired',
        );
        expect(implementation).toHaveLength(1);
      });
    });
  ...
  describe('boundary confidence', () => {
    it('adds near-boundary when mock target path is ambiguous (unresolved internal)', () => {
      const observations: TestObservation[] = [
        makeObservation('MOCK_DECLARATION', 'test.spec.ts', 5, {
          target: './someModule',
        }),
      ...
      const mockAssessments = result.assessments.filter(
        a => a.kind === 'MOCK_INTERNAL_VIOLATION' || a.kind === 'MOCK_BOUNDARY_COMPLIANT',
      );
      expect(mockAssessments.length).toBeGreaterThanOrEqual(1);
      expect(mockAssessments[0].rationale.some(r => r.includes('[near-boundary]'))).toBe(true);
    });

    it('does not add near-boundary for clearly classified boundary mock', () => {
      const observations: TestObservation[] = [
        makeObservation('MOCK_DECLARATION', 'test.spec.ts', 5, {
          target: 'next/router',
        }),
      ...
      const mockAssessments = result.assessments.filter(a => a.kind === 'MOCK_BOUNDARY_COMPLIANT');
      expect(mockAssessments).toHaveLength(1);
      expect(mockAssessments[0].rationale.every(r => !r.includes('[near-boundary]'))).toBe(true);
    });

    it('adds near-boundary for domain boundary mock classification', () => {
      const observations: TestObservation[] = [
        makeObservation('MOCK_DECLARATION', 'test.spec.ts', 5, {
          target: '@/services/hooks/queries/teams',
          resolvedPath: 'src/ui/services/hooks/queries/teams',
        }),
        makeObservation('MOCK_TARGET_RESOLVED', 'test.spec.ts', 5, {
          target: '@/services/hooks/queries/teams',
          resolvedPath: 'src/ui/services/hooks/queries/teams/useTeamsQuery.ts',
          exportNames: ['useTeamsQuery'],
          fileExtension: '.ts',
        }),
      ];

      // Different domain dir so it hits MOCK_DOMAIN_BOUNDARY
      const result = interpretTestQuality(observations, astConfig, 'src/ui/page_blocks/dashboard/users');

      const domainBoundary = result.assessments.filter(a => a.kind === 'MOCK_DOMAIN_BOUNDARY');
