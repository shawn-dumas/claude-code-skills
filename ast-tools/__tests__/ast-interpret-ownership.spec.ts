...
describe('ast-interpret-ownership', () => {
  ...
  describe('negative fixture', () => {
    it('produces zero assessments for non-component file', () => {
      const inventory = analyzeReactFile(fixturePath('interpret-ownership-negative.tsx'));

      // File has no React components, so no component observations
      const componentDecls = inventory.componentObservations.filter(o => o.kind === 'COMPONENT_DECLARATION');
      expect(componentDecls).toHaveLength(0);
  ...
  describe('boundary confidence', () => {
    it('adds near-boundary for AMBIGUOUS classification', () => {
      const componentObservations: ComponentObservation[] = [
        makeComponentDecl('test.tsx', 1, 'AmbiguousComponent'),
        makePropField('test.tsx', 2, 'AmbiguousComponent', 'data'),
      ];

      const hookObservations: HookObservation[] = [makeHookCall('test.tsx', 5, 'useAuthState', 'AmbiguousComponent')];

      const hookAssessments: HookAssessment[] = [
        makeHookAssessment('test.tsx', 5, 'useAuthState', 'LIKELY_CONTEXT_HOOK', 'high'),
      ];
      ...
      // 1 container signal (context hook) + props -> LEAF_VIOLATION
      // The component has leaf evidence AND a disallowed hook
      const assessment = result.assessments[0];
      // Exactly 1 violation-level hook should trigger near-boundary
      expect(
        assessment.rationale.some(r => r.includes('[near-boundary]') && r.includes('1 violation-level hook')),
      ).toBe(true);
    });

    it('adds near-boundary for exactly 1 violation-level hook in LEAF_VIOLATION', () => {
      const componentObservations: ComponentObservation[] = [
        makeComponentDecl('test.tsx', 1, 'LeafComponent'),
        makePropField('test.tsx', 2, 'LeafComponent', 'data'),
      ];

      const hookObservations: HookObservation[] = [
        makeHookCall('test.tsx', 5, 'useUsersQuery', 'LeafComponent', {
          importSource: 'src/ui/services/hooks/queries/users',
        }),
      ];

      const hookAssessments: HookAssessment[] = [
        makeHookAssessment('test.tsx', 5, 'useUsersQuery', 'LIKELY_SERVICE_HOOK', 'high'),
      ];
    ...
    it('does not add violation-hook near-boundary for CONTAINER with many hooks', () => {
      const componentObservations: ComponentObservation[] = [
        makeComponentDecl('containers/test.tsx', 1, 'BigContainer'),
      ];

      const hookObservations: HookObservation[] = [
        makeHookCall('containers/test.tsx', 5, 'useUsersQuery', 'BigContainer', {
          importSource: 'src/ui/services/hooks/queries/users',
        }),
        makeHookCall('containers/test.tsx', 6, 'useAuthState', 'BigContainer'),
        makeHookCall('containers/test.tsx', 7, 'useRouter', 'BigContainer'),
      ];

      const hookAssessments: HookAssessment[] = [
        makeHookAssessment('containers/test.tsx', 5, 'useUsersQuery', 'LIKELY_SERVICE_HOOK', 'high'),
        makeHookAssessment('containers/test.tsx', 6, 'useAuthState', 'LIKELY_CONTEXT_HOOK', 'high'),
      ];
      ...
      expect(result.assessments).toHaveLength(1);
      expect(result.assessments[0].kind).toBe('CONTAINER');
      // CONTAINER should NOT get the 1-violation-hook near-boundary
      expect(result.assessments[0].rationale.every(r => !r.includes('1 violation-level hook'))).toBe(true);
    });
  });
});
