...
describe('ast-type-safety', () => {
  ...
  describe('AS_UNKNOWN_AS (double cast)', () => {
    ...
    it('records hasJustification: false when no comment precedes the cast', () => {
      ...
      // The first double cast has no justification comment on the line above
      const unjustified = doubleCasts.find(o => o.evidence.hasJustification === false);
      expect(unjustified).toBeDefined();
      expect(unjustified!.evidence.castTarget).toBe('Bar');
    });

    it('records hasJustification: true when a comment precedes the cast', () => {
      const observations = extractTypeSafetyObservations(fixturePath('type-safety-violations.ts'));
      const doubleCasts = observationsOfKind(observations, 'AS_UNKNOWN_AS_CAST');

      // The second double cast has a justification comment on the line above
      const justified = doubleCasts.find(o => o.evidence.hasJustification === true);
      expect(justified).toBeDefined();
    });

    it('does not treat JSDoc closing line as justification', () => {
      const observations = extractTypeSafetyObservations(fixturePath('type-safety-violations.ts'));
      const doubleCasts = observationsOfKind(observations, 'AS_UNKNOWN_AS_CAST');

      // The third double cast is after a JSDoc block -- */ is not a justification
      const unjustified = doubleCasts.filter(o => o.evidence.hasJustification === false);
      expect(unjustified.length).toBeGreaterThanOrEqual(2);
    });
  });
