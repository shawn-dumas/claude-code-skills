...
describe('ast-interpret-dead-code', () => {
  ...
  describe('fragility indicator', () => {
    it('marks export with exactly 1 consumer as fragile', () => {
      const observations: ImportObservation[] = [
        {
      ...
      const graph = {
        files: [],
        edges: [{ from: 'consumer.ts', to: 'utils.ts', specifiers: ['helperFn'] }],
        circularDeps: [],
        deadExports: [],
      };

      const result = interpretDeadCode(observations, graph);

      const fragileAssessments = result.assessments.filter(a => a.rationale.some(r => r.includes('[fragile]')));
      expect(fragileAssessments).toHaveLength(1);
      expect(fragileAssessments[0].rationale[0]).toContain('single consumer');
      expect(fragileAssessments[0].rationale[0]).toContain('consumer.ts');
    });

    it('does not mark export with 2+ consumers as fragile', () => {
      const observations: ImportObservation[] = [
        {
          kind: 'EXPORT_DECLARATION',
          file: 'utils.ts',
      ...
      const graph = {
        files: [],
        edges: [
          { from: 'consumer1.ts', to: 'utils.ts', specifiers: ['helperFn'] },
          { from: 'consumer2.ts', to: 'utils.ts', specifiers: ['helperFn'] },
        ],
        circularDeps: [],
        deadExports: [],
      };
      ...
      const fragileAssessments = result.assessments.filter(a => a.rationale.some(r => r.includes('[fragile]')));
      expect(fragileAssessments).toHaveLength(0);
    });

    it('does not emit fragility when no graph is provided', () => {
      const observations: ImportObservation[] = [
        {
          kind: 'EXPORT_DECLARATION',
          file: 'utils.ts',
