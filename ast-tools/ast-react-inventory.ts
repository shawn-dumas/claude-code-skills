...
export function analyzeReactFile(filePath: string): ReactInventory {
  ...
  // Also analyze hook calls in hook definitions (not components)
  // Hook definitions are functions that start with use* but are not components
  for (const hookName of hookDefinitions) {
    const body = localHookBodies.get(hookName);
    if (!body) continue;
    // Skip if already captured as a component (unlikely but safe)
    if (components.some(c => c.name === hookName)) continue;
    // We don't add hooks to the components array -- they are listed separately.
    // The prompt says hookDefinitions is just the names.
  }
