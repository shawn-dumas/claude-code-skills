...
function main(): void {
  ...
  const allResults: FeatureFlagAnalysis[] = [];
  ...
  for (const targetPath of args.paths) {
    ...
    if (stat.isDirectory()) {
      allResults.push(
        ...analyzeFeatureFlagsDirectory(targetPath, { noCache, filter: testFiles ? 'test' : 'production' }),
      );
    } else {
      const result = cached('ast-feature-flags', absolute, () => analyzeFeatureFlags(targetPath), { noCache });
