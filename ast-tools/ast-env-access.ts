...
export function analyzeEnvAccessDirectory(
  dirPath: string,
  ...
  for (const fp of filePaths) {
    const analysis = cached('ast-env-access', fp, () => analyzeEnvAccess(fp), options);
...
function main(): void {
  ...
  if (args.help) {
    process.stdout.write(
      'Usage: npx tsx scripts/AST/ast-env-access.ts <path...> [--pretty] [--no-cache] [--test-files] [--kind <kind>] [--count]\n' +
        '\n' +
  ...
  const allResults: EnvAccessAnalysis[] = [];
  ...
  for (const targetPath of args.paths) {
    ...
    if (stat.isDirectory()) {
      allResults.push(...analyzeEnvAccessDirectory(targetPath, { noCache, filter: testFiles ? 'test' : 'production' }));
    } else {
      const result = cached('ast-env-access', absolute, () => analyzeEnvAccess(targetPath), { noCache });
