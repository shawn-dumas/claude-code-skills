...
import { getFilesInDirectory, getContainingFunctionName, truncateText } from './shared';
...
function main(): void {
  ...
  const allResults: BehavioralAnalysis[] = [];
  ...
  for (const targetPath of args.paths) {
    ...
    if (stat.isDirectory()) {
      allResults.push(
        ...analyzeBehavioralDirectory(targetPath, { noCache, filter: testFiles ? 'test' : 'production' }),
      );
    } else {
      const result = cached('behavioral', absolute, () => analyzeBehavioral(absolute), { noCache });
