...
/**
 * Check if a comment exists on the same line as the cast or the line above.
 * Used to detect whether an `as unknown as` cast has a justification comment
 * as required by P6.
 */
function hasPrecedingComment(sf: SourceFile, castLine: number): boolean {
  const fullText = sf.getFullText();
  const lines = fullText.split('\n');

  // Check the line above the cast (1-indexed, so castLine-2 is the line above).
  // Only match standalone comment lines (// or /* ... */), not JSDoc
  // continuation lines (* @param) or closing lines (*/).
  if (castLine >= 2) {
    const lineAbove = lines[castLine - 2]?.trim() ?? '';
    if (lineAbove.startsWith('//') || (lineAbove.startsWith('/*') && !lineAbove.startsWith('/**'))) {
      return true;
    }
  }

  // Check if the cast line itself has a trailing inline comment after the cast
  const castLineText = lines[castLine - 1] ?? '';
  if (/as unknown as\s+\S.*\/\//.test(castLineText) || /as unknown as\s+\S.*\/\*/.test(castLineText)) {
    return true;
  }
...
function extractObservationFromNode(node: Node, sf: SourceFile, relativePath: string): TypeSafetyObservation | null {
  ...
  if (Node.isAsExpression(node)) {
    ...
    if (Node.isAsExpression(innerExpr)) {
      const innerType = innerExpr.getTypeNode()?.getText();
      if (innerType === 'unknown') {
        // Check if a justification comment precedes the cast (same line or line above)
        const hasJustification = hasPrecedingComment(sf, line);
...
export function analyzeTypeSafetyDirectory(
  dirPath: string,
