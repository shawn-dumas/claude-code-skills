...
// --- AS_UNKNOWN_AS (double cast, no justification) ---
interface Bar {
  x: number;
}
const doubleCast = 'hello' as unknown as Bar;

// --- AS_UNKNOWN_AS (double cast, WITH justification comment) ---
// Partial user with only getIdToken -- fetchApi needs a token but the full shape is not required
const justifiedCast = { getIdToken: () => 'token' } as unknown as Bar;

// --- AS_UNKNOWN_AS (double cast, after JSDoc close -- should NOT count as justified) ---
/**
 * Some function docs.
 */
const afterJsdocCast = 'test' as unknown as Bar;
